...
/**
 * Routes for ESI CA BOT implentation
 */
var ticketingToolRoutes = require("../../routes/TicketingToolRoutes/ticketingToolRoutes");
var customBotActionRoutes = require("../../routes/ESICustomBotActionRoutes/ESICustomBotActionRoutes");
var JWTAuthRoutes = require("../../routes/JWTAuthRoutes/jwtAuthRoutes");
var enterpriseRoutes = require("../../routes/ESIEnterpriseRoutes/ESIEnterpriseRoutes");
...
Application.prototype.attachaccesslogger = function () {
  ...
  if (this.config.app.accesslogs && this.config.app.accesslogger) {
    ...
    morgan.format(
      "combined-trace",
      ':remote-addr - :remote-user [:date[clf]] ":method :url HTTP/:http-version" :status :content-length :res[content-length] :pid(pid) :requestcs(cs) :requestsr(sr)  :requestdelay(d) :response-time ms :requestid(rid) ":referrer" ":user-agent"'
    );
    var stream = {
...
Application.prototype.load = function () {
  ...
  this.app.use("/api/ticketing-tool", ticketingToolRoutes);
  this.app.use("/api/custom-bot-action", customBotActionRoutes);
  this.app.use("/api/enterprise-service", enterpriseRoutes);
  this.app.use("/api/auth-service", JWTAuthRoutes);
